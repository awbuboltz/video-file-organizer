(function ($) {
    ...
    function navigate(path, filterVideos) {
        filterVideos = typeof filterVideos !== 'undefined' ? filterVideos : false;

        $.get(`/files?path=${path}&filterVideos=${filterVideos}`).then(data => {
            renderData(data);
            currentPath = path;
        });
    }
